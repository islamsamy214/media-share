      ...
      >
        ...
        <tbody>
          ...
          >
            ...
            <td class="text-center">{{ blogCategory.blogs_count }}</td>
                ...
                <template #>
                  <router-link
                    ...
                    v-if="
                      user.permissions.some(
                        (permission) =>
                          permission.name == 'update_blog_categories'
                      )
                    "
                  >
                  ...
                  <button
                    ...
                    v-if="
                      user.permissions.some(
                        (permission) =>
                          permission.name == 'delete_blog_categories'
                      )
                    "
                    @click="askDeleteBlogCategory(blogCategory.id)"
                  >
                    <div class="flex gap-2 items-center">
...
export default {
  props: ["blogCategories", "isEmpty", "currentPage", "firstPage", "lastPage"],
  emits: ["previuosPage", "nextPage", "reloadBlogCategorys"],
  ...
  methods: {
    askDeleteBlogCategory(blogCategoryId) {
    ...
    deleteBlogCategory() {
      this.loading = true;
      axios
        .delete(`/api/admin/blogcategories/${this.intendedIdToDelete}`)
        .then((response) => {
          this.$emit("reloadBlogCategorys");
          //flash message
