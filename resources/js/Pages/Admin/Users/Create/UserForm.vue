<template>
  ...
  >
    ...
    <div class="md:col-span-full contain-section">
        ...
        >
          <span
            class="
              ...
              cursor-pointer
            "
            :class="{ 'active-tab': activeTab == model.value }"
            @click="activeTab = model.value"
            >{{ model.name }}</span
          >
        </li>
      </ul>
      <div
        ...
        v-for="model in models"
        :key="model.value"
        :class="{ hidden: activeTab != model.value }"
      >
        <div class="m-3" v-for="permission in permissions" :key="permission">
          <label class="inline-flex items-center">
            <input
              ...
              "
              @change="addPermission($event, permission, model.value)"
            />
            <span class="ml-2">{{ permission + " " + model.name }}</span>
          </label>
        </div>
      </div>
      ...
      <small
        class="errors text-secondary mt-4"
        v-if="form.errors && form.errors.permissions"
      >
...
export default {
  ...
  methods: {
    ...
    addPermission(event, permission, model) {
    ...
    submitUser() {
      ...
      this.loading = true;
      axios
        .post("/api/admin/users", this.generateFormData())
        .then((response) => {
